...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    mkfifo(myfifo, 0644);
     ...
     char temp[30];
    cout << "after this line fifo will be opened for read only"<<endl;
     unnamedPipe = open(myfifo,O_RDONLY);//open fifo for read only
	cout << "if you see this message than read only fifo is opened "<< endl;
    /*
    ...
    */
    ...
    for (int i=0; i<=processNum; i++) {
        int how_many_bytes_actually_read = read(unnamedPipe, temp, 8);
        stringstream splitmsg(temp);
        splitmsg >> p_index >> p_pid;
        id = stoi(p_index.substr(1));
        pid = stoi(p_pid);
        cout << "number of bytes read is  " << how_many_bytes_actually_read<<  endl;
        cout << "for id " << id << " pid of " << pid << " assigned \n";
        pidList[id] = pid;
    ...
    */
    ...
    if (inst_file.is_open()) {
        while ( getline(inst_file, inst)){
            ...
            cout << "current instruction is " << token1 <<  " " << token2 << "  \n";
            /*
            ...
            */
            if (token1.substr(0,3) == "SIG" ) {
                ...
                cout << pidList[tempID] << " nolu process " << tempSig <<" adli sinyal gonderildi ve tempSig"<<tempSig <<endl;
                /*
                ...
                */
                if (tempID == 1 && tempSig == 15) {
                    ...
                    */
                    ...
                    cout << "SIGTERM fifo will read it from here aldo tempSig is"<<tempSig <<endl;
                    read(unnamedPipe, temp, 30);
                    stringstream splitmsg(temp);
                    splitmsg >> p_index >> p_pid;
                    id = stoi(p_index.substr(1));
                    pid = stoi(p_pid);
                    cout << "for id " << id << " pid of " << pid << " assigned " << endl; 
                    pidList[id] = pid;
                }
            }
                /*
